...
export class CacheStrategist {
  ...
  async cacheAppShell(appShell: string[]) {
    await CacheStrategist.cacheAppShell(this.cacheName, appShell);
  }

  async cacheFirst(request: Request) {
    return await CacheStrategist.cacheFirst(request, this.cacheName);
  }

  async staleWhileRevalidate(request: Request) {
    return await CacheStrategist.staleWhileRevalidate(request, this.cacheName);
  }
